...
if choice == "Tablero de Mando":
    ...
    categorias_rep = sorted([
        "Eléctrico", "Mecánico", "Hidráulico", "Neumático",
        "Lubricantes", "Filtros", "Correas y Cadenas",
        "Rodamientos", "Electrónico", "Herramientas", "Otros"
    ])
    ...
    with tab_stock:
        if df_rep.empty:
            ...
            filtro_cat_r = cf1.selectbox(
                "Filtrar Categoría",
                ["Todas"] + categorias_rep
            )
            filtro_stock = cf2.selectbox(
                "Filtrar Estado Stock",
                ["Todos", "OK", "Bajo Stock", "Sin Stock"]
            )
            ...
            if filtro_stock == "OK":
                ...
                df_rep_f = df_rep_f[
                    (df_rep_f['stock_actual'] <= df_rep_f['stock_minimo']) &
                    (df_rep_f['stock_actual'] > 0)
                ]
            ...
            st.dataframe(
                df_rep_f[[
                    'id', 'foto_url', 'nombre', 'referencia',
                    'categoria', 'stock_actual', 'stock_minimo',
                    'unidad', 'ubicacion', 'Estado'
                ]],
                column_config={
                    "foto_url":      st.column_config.ImageColumn("Foto", width="small"),
                    "id":            st.column_config.NumberColumn("ID", format="%d", width="small"),
    ...
    with tab_nuevo:
        ...
        with st.form("form_nuevo_repuesto", clear_on_submit=True):
            ...
            unidad  = c5.selectbox("Unidad", [
                "Unidad", "Par", "Caja", "Litro",
                "Galón", "Metro", "Kilogramo", "Rollo"
            ])

            foto_r = st.file_uploader(
                ...
                if not nom_r:
                    ...
                    try:
                        ...
                        supabase.table("repuestos").insert({
                            ...
                            "categoria":    cat_r,
                            "ubicacion":    ubic_r,
                            "stock_actual": int(stock_i),
                            "stock_minimo": int(stock_m),
                            "unidad":       unidad,
                            "foto_url":     url_foto_r
                        }).execute()
    ...
    with tab_movimientos:
        ...
        if df_rep.empty:
            ...
            if not df_ord.empty:
                ...
                ord_dict  = {
                    f"OT #{r['id']} — {r['descripcion'][:40]}": r['id']
            ...
            with st.form("form_movimiento", clear_on_submit=True):
                ...
                # Vincular a orden solo si es salida
                orden_sel = None
                if tipo_mov == "Salida" and ord_dict:
                    orden_sel = c2.selectbox(
                ...
                observacion = st.text_input(
                    "Observación",
                    placeholder="Ej: Cambio por desgaste en línea 1"
                )
                ...
                if st.form_submit_button("✅ REGISTRAR MOVIMIENTO", type="primary", use_container_width=True):
                    try:
                        ...
                        # Validar que haya stock si es salida
                        if tipo_mov == "Salida" and cantidad > stock_hoy:
                            agregar_notificacion(
                                'error',
                                f'Stock insuficiente. Disponible: {stock_hoy} {rep_actual["unidad"]}'
                            )
                        else:
                            ...
                            nuevo_stock = (
                                stock_hoy - cantidad
                                if tipo_mov == "Salida"
                                else stock_hoy + cantidad
                            )

                            # Determinar orden vinculada
                            id_orden_mov = None
                            ...
                            # 2. Actualizar stock
                            supabase.table("repuestos").update({
                                "stock_actual": nuevo_stock
                            }).eq("id", rep_id).execute()

                            # 3. Alerta si queda bajo mínimo
                            ...
                            if nuevo_stock <= stock_min:
                                mensaje_tel = (
                                    ...
                                    f"Se recomienda realizar una compra."
                                )
                                # Notificar al admin
                                df_admins = df_users[df_users['rol'] == 'Admin'] if not df_users.empty else pd.DataFrame()
                                # Enviamos a todos los admins que tengan chat_id
                            ...
                            agregar_notificacion(
                                'success',
                                f'{tipo_mov} de {cantidad} {rep_actual["unidad"]} de {rep_sel} registrada.'
                            )
                            st.rerun()
                    except Exception as e:
                        agregar_notificacion('error', f'Error: {e}')
    ...
    with tab_alertas:
        ...
        if df_rep.empty:
            ...
            df_alertas_r = df_rep[
                df_rep['stock_actual'] <= df_rep['stock_minimo']
            ].copy()
            ...
            if df_alertas_r.empty:
                ...
                if not df_sin.empty:
                    ...
                    st.dataframe(
                        df_sin[['nombre', 'referencia', 'categoria', 'stock_actual', 'stock_minimo', 'Déficit', 'ubicacion']],
                        hide_index=True,
                        use_container_width=True
                    )
                ...
                if not df_bajo.empty:
                    ...
                    st.dataframe(
                        df_bajo[['nombre', 'referencia', 'categoria', 'stock_actual', 'stock_minimo', 'Déficit', 'ubicacion']],
                        hide_index=True,
                        use_container_width=True
                    )

                # Botón para notificar al admin manualmente
                st.markdown("---")
